...
use FrontBundle\Entity\Schedule;
...
class LoadDataListener
{
    ...
    public function loadData(CalendarEvent $calendarEvent )
    {
        ...
        // $currentEvents = $repository->findByStartDate($startDate);
            ...
            $repository = $this->em->getRepository('FrontBundle:Schedule');
            $schedules = $repository->findAll();

            // You may want to add an Event into the Calendar view.
            /** @var Schedule $schedule */
            foreach ($schedules as $schedule) {
                $calendarEvent->addEvent(new Event($schedule->getTitle(), $schedule->getStart(),$schedule->getId(),$schedule->getEdepart()));
                //
            }
